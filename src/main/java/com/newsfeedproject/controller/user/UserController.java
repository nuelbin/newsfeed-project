...
public class UserController {
	...
	// 회원 단건 조회
	@GetMapping("/{user_id}")
	public ResponseEntity<FindUserByIdResponseDto> userFindByIdAPI(
		@PathVariable("user_id") Long userId) {
		FindUserByIdResponseDto findUserById = userService.findUserByIdService(userId);
	...
	public ResponseEntity<UpdateUserResponseDto> userUpdateAPI(
		@RequestBody UpdateUserRequestDto updateUserRequestDto, HttpServletRequest request) {

		HttpSession session = request.getSession();
		Long userId = (Long)session.getAttribute(SessionConst.LOGIN_USER_ID);

		UpdateUserResponseDto updateUserResponseDto = userService.updateUserService(userId, updateUserRequestDto);
